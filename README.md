# -[README.md](https://github.com/user-attachments/files/25456358/README.md)
<p align="center">
  ...
  <br>HGDoll 是一款 AI 陪玩应用，支持手机端和电脑网页端游戏.
</p>

这是一款完全开源的 AI 陪玩应用。在你游戏时，HGDoll 可实时看到你的游戏画面，陪你聊天、为你加油鼓劲，带来有趣的陪伴体验。它基于豆包大模型和火山方舟 Arkitect 构建，包含[安卓客户端](android/README.md)（Kotlin）、[网页端插件](web-plugin/README.md)（Chrome Extension）和[后端服务](server/README.md)（Python）三部分，支持本地运行轻松上手。
...
graph TD
    ...
    subgraph WebClient[网页端插件]
        WebPlugin --> WebMic[Web Audio API 录音]
        WebPlugin --> WebScreen[chrome.tabs.captureVisibleTab]
        WebMic --> ASRProxy[服务端 ASR 代理]
        ASRProxy --> TextResult2[语音转文字结果]
        WebScreen --> WebScreenshot[定时截图]
        WebAudioPlay[语音播放] --> WebPlugin
    end
    ...
    style WebClient fill:#fbe4f7,stroke:#333,stroke-width:1px
    ...
    style WebPlugin fill:#fff,stroke:#333,stroke-width:1px
...
HGDoll/
├── android/          # 安卓客户端
├── web-plugin/       # 网页端 Chrome 浏览器扩展插件（新增）
├── server/           # 后端服务
└── docs/             # 项目文档
```
